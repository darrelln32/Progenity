...
# set up frames to make them global, then we can grab the values from the variables
frame1 = Frame()
frame2 = Frame()
frame3 = Frame()
frame4 = Frame()
...
time = time.replace('.','_')
filename_str = 'c:\RESULTS\BIG_BOX__' + date + '__' + time + '.csv'
...
square = pygame.Rect(200,440,400,400)
...
while running:
    ...
    if event.type == pygame.QUIT:
        ...
        with open(filename_str,'w') as output_csv:
            ...
            writer.writeheader()
            writer.writerow({'TASK':'BIG BOX','DATE':date,'START':time,'END':stop_time})
            output_csv.write('\n')
