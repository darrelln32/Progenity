...
# When the box is touched, there is a delay period followed by another box that will flash in 1 of 6
# locations on the screen.
# If the second box is touched, the primate receives an award via a hardware system to which the tablet is attached.
...
time = time.replace('.','_')
filename_str = 'c:\RESULTS\SINGLE_POPOUT__' + date + '__' + time + '.csv'


# define the center and radius for each ball
circle_radius = 50
target_radius = target_size / 2
center =[ [400,640],[700,640],[550,345],[250,345],[100,640],[250,940],[550,940] ]

# define the boxes to be touched/to be distractors
...
while running:
    ...
    if event.type == pygame.QUIT:
        ...
        with open(filename_str,'w') as output_csv:
            ...
            writer.writeheader()
            writer.writerow({'TASK':'Single Popout Task','DATE':date,'START':time,'END':stop_time})
            output_csv.write('\n')
    ...
    # is the popout box. from here we will determine the outcome of the trial once
